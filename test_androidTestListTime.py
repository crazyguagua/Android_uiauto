...
import random
import time
from com.comFunction_test import *
import csv
...
class test_Main(findFunction):
    def startTest(self):
        ...
        #             "//*[@resource-id='com.qianmi.cash:id/textview_store_name'][@text='橙子哎_拉卡拉网关真实支付']", 1)
        #     except Exception:
        #         if self.findElement_ByXpath("//*[@resource-id='com.qianmi.cash:id/textview_title'][@text='请选择店铺']", 2):
        ...
        #     pass
        try:
            if self.findElement_ByXpath("//*[@class='android.widget.Toast']", 30).text == "同步成功":
                # 关闭自动打印
        ...
        while True:
            # 点击商品列表分类
            goodsList = self.findElement_ByXpath("//*[@resource-id='com.qianmi.cash:id/textview_category'][@text='位']",
                                                 30)
            goodsList.click()
            ...
            print("总金额" + str(float("%.1f" % totalMoney)))
            # time.sleep(2)
            try:
